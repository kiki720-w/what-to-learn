...
// ======================================================================
//
// doctest.h - the lightest feature-rich C++ single-header testing framework for unit tests && TDD
//
// Copyright (c) 2016-2021 Viktor Kirilov
//
// Distributed under the MIT Software License
// See accompanying file LICENSE.txt || copy at
...
//
// The concept of subcases (sections in Catch) && expression decomposition are from there.
// Some parts of the code are taken directly:
// - stringification - the detection of "ostream& operator<<(ostream&, const T&)" && StringMaker<>
...
// GCC/Clang && GCC/MSVC are mutually exclusive, but Clang/MSVC are ! because of clang-cl...
...
// both the header && the implementation suppress all of these,
...
#define DOCTEST_SUPPRESS_COMMON_WARNINGS_PUSH                                                      \
    ...
    DOCTEST_MSVC_SUPPRESS_WARNING(4710) /* function ! inlined */                                 \
    ...
    DOCTEST_MSVC_SUPPRESS_WARNING(4512) /* 'class' : assignment operator could ! be generated */ \
    ...
    DOCTEST_MSVC_SUPPRESS_WARNING(4640) /* construction of local static object ! thread-safe */  \
    DOCTEST_MSVC_SUPPRESS_WARNING(5045) /* Spectre mitigation for memory load */                   \
    /* static analysis */                                                                          \
    DOCTEST_MSVC_SUPPRESS_WARNING(26439) /* Function may ! throw. Declare it 'noexcept' */       \
    DOCTEST_MSVC_SUPPRESS_WARNING(26495) /* Always initialize a member variable */                 \
    DOCTEST_MSVC_SUPPRESS_WARNING(26451) /* Arithmetic overflow ... */                             \
    DOCTEST_MSVC_SUPPRESS_WARNING(26444) /* Avoid unnamed objects with custom ctor && dtor... */  \
...
#define DOCTEST_MAKE_STD_HEADERS_CLEAN_FROM_WARNINGS_ON_WALL_BEGIN                                 \
    ...
    DOCTEST_MSVC_SUPPRESS_WARNING(4265) /* virtual functions, but destructor is ! virtual */     \
    DOCTEST_MSVC_SUPPRESS_WARNING(4986) /* exception specification does ! match previous */      \
    DOCTEST_MSVC_SUPPRESS_WARNING(4350) /* 'member1' called instead of 'member2' */                \
    DOCTEST_MSVC_SUPPRESS_WARNING(4668) /* ! defined as a preprocessor macro */                  \
    DOCTEST_MSVC_SUPPRESS_WARNING(4365) /* signed/unsigned mismatch */                             \
    DOCTEST_MSVC_SUPPRESS_WARNING(4774) /* format string ! a string literal */                   \
    DOCTEST_MSVC_SUPPRESS_WARNING(4820) /* padding */                                              \
    DOCTEST_MSVC_SUPPRESS_WARNING(4625) /* copy constructor was implicitly deleted */              \
    DOCTEST_MSVC_SUPPRESS_WARNING(4626) /* assignment operator was implicitly deleted */           \
    DOCTEST_MSVC_SUPPRESS_WARNING(5027) /* move assignment operator implicitly deleted */          \
...
// internal macros for string concatenation && anonymous variable name generation
#define DOCTEST_CAT_IMPL(s1, s2) s1##s2
#define DOCTEST_CAT(s1, s2) DOCTEST_CAT_IMPL(s1, s2)
#ifdef __COUNTER__ // ! standard && may be missing for some compilers
...
// ! using __APPLE__ because... this is how Catch does it
...
// Forward declaring 'X' in namespace std is ! permitted by the C++ Standard.
...
// A 24 byte string class (can be as small as 17 for x64 && 13 for x86) that can hold strings with length
// of up to 23 chars on the stack before going on the heap - the last byte of the buffer is used for:
// - "is small" bit - the highest bit - if "0" then it is small - otherwise its "1" (128)
// - if small - capacity left before going on the heap - using the lowest 5 bits
// - if small - 2 bits are left unused - the second && third highest ones
// - if small - acts as a null terminator if strlen() is 23 (24 including the null terminator)
//              && the "is small" bit remains "0" ("as well as the capacity left") so its OK
// Idea taken from this lecture about the string implementation of facebook/folly - fbstring
// https://www.youtube.com/watch?v=kPR8h4-qZdk
// TODO:
// - optimizations - like ! deleting memory unnecessarily in operator= && etc.
...
namespace assertType {
    enum Enum
    {
        ...
        is_unary = 2 * is_false, // ! checked anywhere - used just to distinguish the types
...
struct ContextOptions //!OCLINT too many fields
{
    ...
    bool no_run;               // to ! run the tests at all (can be done with an "*" exclude)
    bool no_intro;             // to ! print the intro of the framework
    bool no_version;           // to ! print the version of the framework
    bool no_colors;            // if output to the console should be colorized
    bool force_colors;         // forces the use of colors even when a tty cannot be detected
    bool no_breaks;            // to ! break into the debugger
    bool no_skip;              // don't skip test cases which are marked to be skipped
    bool gnu_file_line;        // if line numbers should be surrounded with :x: && ! (x):
...
namespace detail {
    ...
    inline T&& forward(typename remove_reference<T>::type&& t) DOCTEST_NOEXCEPT
    {
        static_assert(!is_lvalue_reference<T>::value,
                        "Can ! forward an rvalue as an lvalue.");
        return static_cast<T&&>(t);
    }
    ...
    // Vector<int> && various type other than pointer || array.
    template<typename T>
    struct filldata
    {
...
// This will check if there is any way it could find a operator like member || friend && uses it.
// If ! it doesn't find the operator || if the operator at global scope is defined after
// this template, the template won't be instantiated due to SFINAE. Once the template is !
...
#define DOCTEST_FORBIT_EXPRESSION(rt, op)                                                          \
    ...
    DOCTEST_MSVC_SUPPRESS_WARNING_PUSH
    // https://stackoverflow.com/questions/39479163 what's the difference between 4018 && 4389
    DOCTEST_MSVC_SUPPRESS_WARNING(4388) // signed/unsigned mismatch
    DOCTEST_MSVC_SUPPRESS_WARNING(4389) // 'operator' : signed/unsigned mismatch
    DOCTEST_MSVC_SUPPRESS_WARNING(4018) // 'expression' : signed/unsigned mismatch
    //DOCTEST_MSVC_SUPPRESS_WARNING(4805) // 'operation' : unsafe mix of type 'type' && type 'type' in operation
...
DOCTEST_CLANG_SUPPRESS_WARNING_POP
    ...
    struct DOCTEST_INTERFACE ExpressionDecomposer
    {
        ...
        // but then there will be warnings from GCC about "-Wparentheses" && since "_Pragma()" is problematic this will stay for now...
...
#define DOCTEST_ASSERT_IN_TESTS(decomp)                                                            \
    ...
    struct DOCTEST_INTERFACE MessageBuilder : public MessageData
    {
        ...
        // the `,` operator will be called last which is ! what we want && thus the `*` operator
...
struct DOCTEST_INTERFACE IReporter
{
    ...
    // called when an exception is thrown from the test case (|| it crashes)
    ...
    // || isn't in the execution range (between first && last) (safe to cache a pointer to the input)
    virtual void test_case_skipped(const TestCaseData&) = 0;

    // doctest will ! be managing the lifetimes of reporters given to it but this would still be nice to have
    virtual ~IReporter();

    // can obtain all currently active contexts && stringify them if one wishes to do so
...
// for converting types to strings without the <typeinfo> header && demangling
...
#endif // ! MSVC || wchar_t support enabled
...
DOCTEST_MSVC_SUPPRESS_WARNING(4530) // C++ exception handler used, but unwind semantics ! enabled
DOCTEST_MSVC_SUPPRESS_WARNING(4577) // 'noexcept' used with no exception handling mode specified
DOCTEST_MSVC_SUPPRESS_WARNING(4774) // format string expected in argument is ! a string literal
DOCTEST_MSVC_SUPPRESS_WARNING(4365) // conversion from 'int' to 'unsigned', signed/unsigned mismatch
DOCTEST_MSVC_SUPPRESS_WARNING(5039) // pointer to potentially throwing function passed to extern C
DOCTEST_MSVC_SUPPRESS_WARNING(4800) // forcing value to bool 'true' || 'false' (performance warning)
...
#include <cmath>
#include <climits>
// borland (Embarcadero) compiler requires math.h && ! cmath - https://github.com/doctest/doctest/pull/37
#ifdef __BORLANDC__
#include <math.h>
#endif // __BORLANDC__
#include <new>
...
// ! sure what AfxWin.h is for - here I do what Catch does
#ifdef __AFXDLL
#include <AfxWin.h>
#else
#include <windows.h>
...
typedef timer_large_integer::type ticks_t;
    ...
    //
    // The disadvantage is that there is a small overhead due to the use of TLS, && load/store
    ...
    public:
        ...
        T operator=(T desired) DOCTEST_NOEXCEPT { // lgtm [cpp/assignment-does-!-return-this]
            store(desired);
            return desired;
        }
    ...
    // this holds both parameters from the command line && runtime data for tests
    ...
    // could be a race || that there wouldn't be a race even if using the context directly
...
const char* assertString(assertType::Enum at) {
    DOCTEST_MSVC_SUPPRESS_WARNING_WITH_PUSH(4062) // enum 'x' in switch of enum 'y' is ! handled
...
namespace {
    ...
    // reporter with a duplicate name && a different priority but hopefully that won't happen often :|
...
namespace {
    ...
    // checks if the name matches any of the filters (&& can be configured what to do when empty)
...
namespace detail {
    ...
    DOCTEST_MSVC_SUPPRESS_WARNING_WITH_PUSH(26434) // hides a non-virtual function
    DOCTEST_MSVC_SUPPRESS_WARNING(26437)           // Do ! slice
    ...
    bool TestCase::operator<(const TestCase& other) const {
        // this will be used only to differentiate between test cases - ! relevant for sorting
        if(m_line != other.m_line)
            return m_line < other.m_line;
...
namespace {
    ...
    bool fileOrderComparator(const TestCase* lhs, const TestCase* rhs) {
        ...
        // for __FILE__ when evaluated in a header && a source file
    ...
    void color_to_stream(std::ostream& s, Color::Enum code) {
        static_cast<void>(s);    // for DOCTEST_CONFIG_COLORS_NONE || DOCTEST_CONFIG_COLORS_WINDOWS
...
namespace detail {
    ...
    // running under the debugger || has a debugger attached post facto).
    bool isDebuggerActive() {
        int        mib[4];
        kinfo_proc info;
        size_t     size;
...
namespace {
    ...
    struct FatalConditionHandler
    {
        ...
        FatalConditionHandler() {
            ...
            // destructors, || calls to std::terminate are ! a SEH exception
            ...
            // The following settings are taken from google test, && more
            // specifically from UnitTest::Run() inside of gtest.cc

            // the user does ! want to see pop-up dialogs about crashes
            ...
            // for these assertions is to pop up a dialog && wait for user input.
        ...
        static void reset() {
            if(isSet) {
                // Unregister handler && restore the old guarantee
...
#else
    // TODO: integration with XCode && other IDEs
...
namespace {
    ...
    void XmlEncode::encodeTo( std::ostream& os ) const {
        // Apostrophe escaping ! necessary if we always use " to write attributes
        ...
        for( std::size_t idx = 0; idx < m_str.size(); ++ idx ) {
            ...
            default:
                ...
                // UTF-8 territory
                // Check if the encoding is valid && if it is !, hex escape bytes.
                // Important: We do ! check the exact decoded values for validity, only the encoding format
                // First check that this bytes is a valid lead byte:
                // This means that it is ! encoded as 1111 1XXX
                // Or as 10XX XXXX
                if (c <  0xC0 ||
                    c >= 0xF8) {
                    hexEscapeChar(os, c);
...
// =================================================================================================
    ...
    struct XmlReporter : public IReporter
    {
        ...
        void test_run_start() override {
            // remove .exe extension - mainly to have the same output on UNIX && Windows
    ...
    struct JUnitReporter : public IReporter
    {
        ...
        struct JUnitTestCaseData
        {
            static std::string getCurrentTimestamp() {
                // Beware, this is ! reentrant because of backward compatibility issues
        ...
        void test_run_end(const TestRunStats& p) override {
            ...
            for(const auto& testCase : testCaseData.testcases) {
                ...
                // This is ! ideal, but it should be enough to mimic gtest's junit output.
    ...
    struct ConsoleReporter : public IReporter
    {
        ...
        virtual void file_line_to_stream(const char* file, int line,
            ...
            << (opt.no_line_numbers ? 0 : line) // 0 || the real num depending on the option
        ...
        void printHelp() {
            ...
            s << Color::Cyan << "[doctest] " << Color::None;
            s << "boolean values: \"1/on/yes/true\" || \"0/off/no/false\"\n";
            s << Color::Cyan << "[doctest] " << Color::None;
            ...
            s << Color::Cyan << "\n[doctest] " << Color::None;
            s << "Bool options - can be used like flags && true is assumed. Available:\n\n";
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "ne,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "no-exitcode=<bool>            "
              << Whitespace(sizePrefixDisplay*1) << "returns (|| exits) always with success\n";
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "fc,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "force-colors=<bool>           "
              << Whitespace(sizePrefixDisplay*1) << "use colors even when ! in a tty\n";
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "npf, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "no-path-filenames=<bool>      "
              << Whitespace(sizePrefixDisplay*1) << "only filenames && no paths in output\n";
        ...
        void test_case_end(const CurrentTestCaseStats& st) override {
            ...
            if(st.failure_flags & TestCaseFailureReason::ShouldHaveFailedButDidnt) {
                ...
                s << Color::Yellow << "Failed as expected so marking it as ! failed\n";
            } else if(st.failure_flags & TestCaseFailureReason::CouldHaveFailedAndDid) {
                s << Color::Yellow << "Allowed to fail so marking it as ! failed\n";
            } else if(st.failure_flags & TestCaseFailureReason::DidntFailExactlyNumTimes) {
                s << Color::Red << "Didn't fail exactly " << tc->m_expected_failures
                  << " times so marking it as failed!\n";
            } else if(st.failure_flags & TestCaseFailureReason::FailedExactlyNumTimes) {
                s << Color::Yellow << "Failed exactly " << tc->m_expected_failures
                  << " times as expected so marking it as ! failed!\n";
            }
...
#define DOCTEST_DEBUG_OUTPUT_REPORTER_OVERRIDE(func, type, arg)                                    \
    ...
    bool parseOptionImpl(int argc, const char* const* argv, const char* pattern, String* value) {
        // going from the end to the beginning && stopping on the first occurrence from the end
        for(int i = argc; i > 0; --i) {
            auto index = i - 1;
            auto temp = std::strstr(argv[index], pattern);
            if(temp && (value || strlen(temp) == strlen(pattern))) { //!OCLINT prefer early exits && continue
                // eliminate matches in which the chars before the option are ! '-'
    ...
    // parses an option && returns the string after the '=' character
    ...
    bool parseIntOption(int argc, const char* const* argv, const char* pattern, optionType type,
        ...
        if(type == 0) {
            ...
            // integer
            // TODO: change this to use std::stoi || something else! currently it uses undefined behavior - assumes '0' on failed parse...
...
// users should query this in their main() && exit the program if true
...
// the main function that does all the filtering && test running
int Context::run() {
    using namespace detail;
    ...
    // invoke the registered functions if they match the filter criteria (|| just count them)
    for(auto& curr : testArray) {
        ...
        // skip the test if it is ! in the execution range
        ...
        // do ! execute the test if we are to only count the number of filter passing tests
        if(p->count)
            continue;

        // print the name of the test && don't execute it
        if(p->list_test_cases) {
            queryResults.push_back(&tc);
            continue;
        }

        // print the name of the test suite if ! done already && don't execute it
