...
Node::Node(Config _C, DistTable& D, Node* _parent,
           const std::vector<float>* pressure, float pressure_weight)
    : C(_C),
      parent(_parent),
      priorities(C.size(), 0),
      order(C.size(), 0),
      search_tree(std::queue<Constraint*>())
{
  ...
  std::sort(order.begin(), order.end(), [&](int i, int j) {
    const auto pressure_i =
        pressure == nullptr ? 0.0f : pressure_weight * (*pressure)[C[i]->id];
    const auto pressure_j =
        pressure == nullptr ? 0.0f : pressure_weight * (*pressure)[C[j]->id];
    return priorities[i] + pressure_i > priorities[j] + pressure_j;
  });
}
...
Planner::Planner(const Instance* _ins, const Deadline* _deadline,
                 std::mt19937* _MT_rng, int _verbose,
                 const std::vector<float>* _pressure, float _pressure_weight)
    : ins(_ins),
      deadline(_deadline),
      MT(_MT_rng),
      verbose(_verbose),
      ...
      pressure(_pressure),
      pressure_weight(_pressure_weight),
      A(Agents(N, nullptr)),
...
Solution Planner::solve()
{
  ...
  // insert initial node
  auto S = new Node(ins->starts, D, nullptr, pressure, pressure_weight);
  OPEN.push(S);
  CLOSED[S->C] = S;
  ...
  while (!OPEN.empty() && !is_expired(deadline)) {
    ...
    auto S_new = new Node(C, D, S, pressure, pressure_weight);
    OPEN.push(S_new);
...
Solution solve(const Instance& ins, const int verbose, const Deadline* deadline,
               std::mt19937* MT, const std::vector<float>* pressure,
               float pressure_weight)
{
  info(1, verbose, "elapsed:", elapsed_ms(deadline), "ms\tpre-processing");
  auto planner = Planner(&ins, deadline, MT, verbose, pressure, pressure_weight);
