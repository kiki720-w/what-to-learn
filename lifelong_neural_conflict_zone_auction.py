...
def conflict_zone_auction_step(
    obs: torch.Tensor,
    current: List[Position],
    dist_maps: List[torch.Tensor],
    previous_positions: List[Position],
...
def run_episode(cfg: LifelongConflictZoneAuctionConfig, use_neural: bool, model=None):
    ...
    t0 = time.time()
    desc = "Neural Conflict-Zone Auction" if use_neural else "Vanilla Conflict-Zone Auction"
    for t in tqdm(range(cfg.TOTAL_STEPS), desc=desc, leave=False):
        ...
        next_pos, info = conflict_zone_auction_step(
            obs=env.obs,
            current=current,
            dist_maps=dist_maps,
...
def run_multi_seed():
    ...
    print("=== Lifelong Neural-Priority Conflict-Zone Auction Experiment ===")
    print("Seeds:", seeds)
    ...
    vanilla_summary = summarize("Vanilla Conflict-Zone Auction Summary", vanilla_results)
    neural_summary = summarize("Neural-Priority Conflict-Zone Auction Summary", neural_results)
