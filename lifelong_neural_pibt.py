...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Lifelong Neural-Priority PIBT experiment runner."
    )
    ...
    parser.add_argument("--w_goal", type=float, default=3.5)
    parser.add_argument("--w_wait", type=float, default=1.0)
    parser.add_argument("--w_conflict", type=float, default=12.0)
    parser.add_argument("--w_congestion", type=float, default=0.5)
    parser.add_argument("--use_heatmap_reward", action="store_true")
...
def config_from_args(args, seed: int) -> "LifelongPIBTConfig":
    return LifelongPIBTConfig(
        ...
        USE_HEATMAP_REWARD=args.use_heatmap_reward,
...
class LifelongPIBTConfig:
    ...
    # Main method:
    # False = heatmap only changes priority ordering
    # True  = heatmap changes priority ordering + candidate movement score
...
def plan_one_step_pibt(
    ...
    Current main method uses USE_HEATMAP_REWARD=False.
    """
    n = len(current)
    ...
    current_owner: Dict[Position, int] = {pos: i for i, pos in enumerate(current)}

    def choose(agent_id: int, visiting: set):
        if agent_id in visiting:
            return False

        if next_pos[agent_id] is not None:
            return True
        ...
        for _, _, cand in scored:
            if cand in reserved and reserved[cand] != agent_id:
                continue

            occupant = current_owner.get(cand)
            if occupant is not None and occupant != agent_id:
                if next_pos[occupant] is None:
                    ok = choose(occupant, visiting)
                    if not ok:
                        continue

                if next_pos[occupant] == cand:
                    continue

                if cand in reserved and reserved[cand] != agent_id:
                    continue

            # avoid edge swap with agents already assigned by inheritance
            swap_conflict = False
...
def run_multi_seed(args=None):
    ...
    if args.mode == "vanilla":
        if all_vanilla:
            summarize_results("Vanilla Lifelong PIBT Summary", all_vanilla)
        return
    if args.mode == "neural":
        if all_neural:
            summarize_results("Neural-Priority Lifelong PIBT Summary", all_neural)
        return
