...
# 4. Load U-Net model
# =====================================================
def load_unet_model(cfg: LifelongSIPPConfig):
...
# =====================================================
# 6. SIPP-style search with reservations
# =====================================================
...
def get_safe_intervals_for_pos(
    pos: Position,
    horizon: int,
    vertex_res: Dict[int, Set[Position]],
):
    blocked = {
        ...
        if is_reserved_vertex(pos, t, vertex_res)
    }
...
def find_safe_interval(intervals, earliest_t: int):
    for idx, (s, e) in enumerate(intervals):
        if e < earliest_t:
            continue
        return idx, max(s, earliest_t), e
    return None


def reconstruct_timed_path(parent, state):
    timed = []
    while state is not None:
        pos, _, t = state
        timed.append((pos, t))
    ...
    path = []
    for idx, (pos, t) in enumerate(timed):
        if idx == 0:
            while len(path) <= t:
                path.append(pos)
            continue

        prev_pos, prev_t = timed[idx - 1]
        while len(path) < t:
...
def sipp_search(
    ...
    """
    Windowed SIPP-style search over discrete safe intervals.

    State:
        (position, safe_interval_id, arrival_time)
    ...
    """
    safe_cache: Dict[Position, List[Tuple[int, int]]] = {}
    open_list = []
    g_score = {}
    parent = {}

    start_intervals = get_safe_intervals_for_pos(start, horizon, vertex_res)
    start_interval = find_safe_interval(start_intervals, 0)
    if start_interval is None:
        return [start for _ in range(horizon + 1)]

    start_interval_id, start_t, _ = start_interval
    start_state = (start, start_interval_id, start_t)
    g_score[start_state] = 0
    parent[start_state] = None

    h0 = float(dist_map[start[0], start[1]])
    heapq.heappush(open_list, (start_t + h0, start_t, random.random(), start_state))

    best_state = start_state
    best_h = h0

    while open_list:
        _, t, _, state = heapq.heappop(open_list)
        pos, interval_id, cur_t = state

        if t != cur_t:
            continue

        if pos not in safe_cache:
            safe_cache[pos] = get_safe_intervals_for_pos(pos, horizon, vertex_res)

        interval_start, interval_end = safe_cache[pos][interval_id]
        ...
        if t >= horizon:
            path = reconstruct_timed_path(parent, state)
            while len(path) < horizon + 1:
                path.append(path[-1])
            return path

        if pos == goal and interval_end >= horizon:
            path = reconstruct_timed_path(parent, state)
            while len(path) < horizon + 1:
                path.append(pos)
            return path
        ...
        for nxt in get_neighbors(pos, obs):
            ...
            if nxt not in safe_cache:
                safe_cache[nxt] = get_safe_intervals_for_pos(nxt, horizon, vertex_res)

            earliest_arrival = t + 1
            interval = find_safe_interval(safe_cache[nxt], earliest_arrival)

            while interval is not None:
                next_interval_id, arrival_t, _ = interval
                departure_t = arrival_t - 1

                if departure_t > interval_end:
                    break

                # If another planned agent moves nxt -> pos at this transition,
                # then pos -> nxt would create an edge swap.
                if is_reserved_edge(nxt, pos, arrival_t, edge_res):
                    interval = find_safe_interval(
                        safe_cache[nxt],
                        arrival_t + 1,
                    )
                    continue

                next_state = (nxt, next_interval_id, arrival_t)
                if next_state not in g_score or arrival_t < g_score[next_state]:
                    g_score[next_state] = arrival_t
                    ...
                    heapq.heappush(
                        open_list,
                        (f, arrival_t, random.random(), next_state),
                    )
                break

        # Explicit wait to the end of the current safe interval. This lets SIPP
        # cross through a cell after a temporary reservation clears elsewhere.
        if interval_end > t:
            wait_t = min(interval_end, horizon)
            wait_state = (pos, interval_id, wait_t)
            if wait_state not in g_score or wait_t < g_score[wait_state]:
                g_score[wait_state] = wait_t
                parent[wait_state] = state
                h = float(dist_map[pos[0], pos[1]])
                heapq.heappush(
                    open_list,
                    (wait_t + h, wait_t, random.random(), wait_state),
                )

            if pos == goal:
                path = reconstruct_timed_path(parent, wait_state)
                while len(path) < horizon + 1:
                    path.append(pos)
                return path

    # Fallback: reconstruct best partial path and pad by waiting
    path = reconstruct_timed_path(parent, best_state)
    if len(path) == 0:
        path = [start]
...
# =====================================================
def plan_one_step_sipp_prioritized(
    obs: torch.Tensor,
    current: List[Position],
    goals: List[Position],
    dist_maps: List[torch.Tensor],
    ...
    """
    SIPP-style prioritized planning one-step planner.
    ...
    Planning:
        Plan each agent in priority order using a discrete safe-interval
        search with a reservation table, over PLAN_HORIZON steps.
...
# =====================================================
# 8. Run one lifelong SIPP method
# =====================================================
def run_lifelong_sipp_method(
    cfg: LifelongSIPPConfig,
    use_neural: bool,
    model=None,
):
    ...
    method_name = (
        "Neural-Priority Lifelong SIPP"
        if use_neural
        else "Vanilla Lifelong SIPP"
    )
    ...
    for t in pbar:
        ...
        next_positions = plan_one_step_sipp_prioritized(
            obs=env.obs,
            current=current,
            goals=env.goals,
            dist_maps=dist_maps,
...
def run_multi_seed():
    ...
    base_cfg = LifelongSIPPConfig(
        H=32,
        W=32,
        N_AGENTS=24,
        TOTAL_STEPS=500,

        # Match the random-obstacle cross-planner protocol used by
        # PP / Token Passing / Push-style / LNS-style comparisons.
    ...
    print("=== Lifelong Neural-Priority SIPP Experiment ===")
    print("Seeds:", SEEDS)
    ...
    for seed in SEEDS:
        ...
        vanilla = run_lifelong_sipp_method(
            cfg=cfg,
            use_neural=False,
            model=None,
        )

        neural = run_lifelong_sipp_method(
            cfg=cfg,
            use_neural=True,
            model=model,
        )
        ...
        print(f"\nSeed {seed} results:")
        print(
            f"Vanilla SIPP tasks={vanilla['completed_tasks']}, "
        ...
        print(
            f"Neural  SIPP tasks={neural['completed_tasks']}, "
    ...
    vanilla_summary = summarize_results(
        "Vanilla Lifelong SIPP Summary",
        all_vanilla,
    )

    neural_summary = summarize_results(
        "Neural-Priority Lifelong SIPP Summary",
        all_neural,
    )
